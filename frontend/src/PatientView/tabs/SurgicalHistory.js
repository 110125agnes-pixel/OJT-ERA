...
const SurgicalHistory = ({ patientId }) => {
  ...
  useEffect(() => {
    ...
    surgicalService
      ...
      .then((data) => {
        ...
        const filtered = mapped.filter((s) => isValidName(s.name));
        setSurgeries(filtered.length ? filtered : defaultSurgeries);
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    surgicalService
      ...
      .then((data) => {
        ...
        const sel =
          data && data.selectedSurgeries ? data.selectedSurgeries : data;
        if (Array.isArray(sel)) {
          ...
          const filteredSel = mapped.filter((s) => isValidName(s.name));
          setSelectedSurgeries(filteredSel);
        }
      })
      .catch((err) => {
        console.warn(
          "No patient surgical history from server, falling back to storage",
          err,
        );
        try {
          const raw = localStorage.getItem("surgicalHistorySelections");
  ...
  const handleCheckboxChange = (surgery) => {
    ...
    if (selectedSurgeries.find((s) => (s.code || s.id) === code)) {
      ...
      const entry = {
        id: code,
        code: code,
        name: surgery.name || surgery.SURGERY_DESC || "",
      };
  ...
  const saveSelections = (selected, none, noteText) => {
    try {
      ...
      const payload = {
        selectedSurgeries: selected,
        noneChecked: none,
        notes: noteText,
      };
      localStorage.setItem(
        "surgicalHistorySelections",
        JSON.stringify(payload),
      );
      // Dispatch a custom event so other components in the same window can react immediately
      try {
        const ev = new CustomEvent("surgicalHistoryUpdated", {
          detail: payload,
        });
      ...
      if (patientId) {
        ...
        surgicalService
          .savePatientSurgicalHistory(patientId, serverItems)
          .catch((err) => {
        ...
        <div className="surgical-history-left">
          ...
          <div className="checkbox-list-surgical">
            ...
            {surgeries.map((surgery) => (
              <div
                key={surgery.id || surgery.code}
                className="checkbox-item-surgical"
              >
                <label>
                  <input
                    type="checkbox"
                    checked={selectedSurgeries.some(
                      (s) =>
                        (s.code || s.id) ===
                        (surgery.code || surgery.SURGERY_CODE || surgery.id),
                    )}
        ...
        <div className="surgical-history-right">
          <div className="notes-section-surgical">
            <textarea
              ...
              rows="3"
            />
            <span className="char-limit-surgical">
              Limit your characters to 2000
            </span>
          </div>

          <div className="action-buttons-top-surgical">
            <button
              className="btn-add-surgical"
            ...
            <table className="surgical-table">
              ...
              <tbody>
                {selectedSurgeries.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="2" className="no-data-cell-surgical">
                      No items selected
                    </td>
                  </tr>
                )}
